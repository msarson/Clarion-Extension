...
- ✨ **Inlay hints**: inline **implicit-variable types** (`Counter#` → `: LONG`, `Pct$` → `: REAL`) and **parameter-name hints** at call sites (`Save(⟨pName:⟩ 'report', ⟨pFlags:⟩ 3)`). Reuses the implicit-variable suffix rule and `Token.parameters`; parameter-name hints are conservative (single unambiguous same-file `MAP` signature). Enabled by upgrading the language client/server to LSP 8.x (which also unlocks other 3.17 features natively).
