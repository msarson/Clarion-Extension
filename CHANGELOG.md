...
- 🐛 **Bare `WINDOW` keyword tokenizes incorrectly** (`BareWin WINDOW` with no parens, no attributes): the keyword's regex required a `(` or `,` lookahead. The bare form failed the lookahead, the tokenizer's no-match advance skipped the leading `W`, and the residual `INDOW` matched as a Variable token — so `BareWin WINDOW \n CODE` produced `Label("BareWin")` + `Variable("INDOW")` instead of a Structure. The lookahead now also accepts whitespace-followed-by-end-of-line and `!` (comment), so the bare form tokenizes correctly while declarations with attributes (`WINDOW('Title'),AT(…)`) remain unchanged.
