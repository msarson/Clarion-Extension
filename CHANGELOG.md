...
- ✨ **Add PROCEDURE to MODULE code action** (issue #87): pressing `Ctrl+.` while inside an existing MODULE block offers **Add PROCEDURE to MODULE**. A procedure name is prompted, the prototype is inserted into the MODULE block, and the implementation stub is appended to the referenced CLW file (which is opened and navigated to).
