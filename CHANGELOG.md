...
- **Renaming a method declared in a library include now says so.** F2 on `TemplateHelper.StoreAppFrameDimensions()` in a generated module, where the class lives in an accessory's include under the Clarion folder, was refused as "symbol not found". The rename is still refused, since the declaration is outside the solution and compiled into a DLL, but the message now names the declaring file and the library folder. A dotted method is resolved through its class before the older checks run, which also lets a method of a class in the solution folder be renamed where the per-file lookup used to give up. [#547](https://github.com/msarson/Clarion-Extension/issues/547)
