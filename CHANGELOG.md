...
- 🐛 **Intermittent "Rename: Failed to apply edits" on a method rename** (#196): `provideRename` now emits versioned `documentChanges` (instead of unversioned `changes`) and dedupes overlapping/duplicate ranges per file, so the active `.clw` implementation edit no longer gets rejected while the `.inc` declaration edit applies (the partial-rename symptom). See GH #196.
