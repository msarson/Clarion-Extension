...
- ✨ **Diagnostic: undeclared variable on LHS of assignment** (issue #62, **opt-in v1**): when enabled via the new `clarion.diagnostics.undeclaredVariables.enabled` setting, raises a Warning on assignments like `Foo = 1` when `Foo` is not declared anywhere in the current file. **Default off** — toggling requires reloading the VS Code window so the language server can pick up the new value. v1 deliberately checks ONLY bare-identifier left-hand sides of assignments (including `+=`, `-=`, `*=`, `/=`); prefixed (`Cus:Field`), dotted (`obj.member`), indexed (`arr[i]`), and field-equate (`?Ctrl`) forms are intentionally not validated. Built-in Clarion identifiers (SELF, PARENT, RECORDS, ERRORCODE, …) are always treated as declared. Resolution is single-file only: cross-file globals declared in another MEMBER aren't resolved in v1, so users with cross-file globals should keep the feature off until a future iteration adds reachability-aware lookup. Diagnostic code: `undeclared-variable`.
