...
- 🧹 **Doc-link refresh handler no-op'd pending languageclient upgrade** ([GH #160](https://github.com/msarson/Clarion-Extension/issues/160)): the fake-edit invalidation (insert+delete a space on every visible editor) flipped VS Code's dirty flag on every doc at every solution-ready. Now a logging stub until `vscode-languageclient` v8/v9 lands so `workspace.documentLink.refreshSupport` capability can be declared properly. Doc-links still resolve and light up after first interaction (focus/edit/save).
