...
- 🐛 **"Rename: Failed to apply edits" on a method rename** (#196): renaming a class method at its `.clw` implementation point partially failed — the `.inc` declaration renamed but the active `.clw` edit was rejected. `provideRename` now emits `documentChanges` with a **null (unversioned)** edit version and dedupes overlapping/duplicate ranges per file. Root cause: attaching the LSP document version to the edit — VS Code's internal text-model version is a separate counter that diverges from the LSP version across undo/redo, so the versioned edit for the open document was rejected on a version mismatch while the closed file (no version) applied. A null version means "apply without a version check", correct for a synchronously-computed rename. See GH #196.
