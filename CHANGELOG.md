...
- 🐛 **Hover spinner no longer hangs indefinitely on CLASS base types** — hovering over a class base-type name (e.g. `ce_MetroWizardForm` in `MetroForm CLASS(ce_MetroWizardForm)`) could spin forever while the class definition index was built lazily on first use. A 10-second timeout guard now ensures the spinner always resolves even if the index is still loading. Combined with the parallel index build above, the hang is eliminated in practice.
