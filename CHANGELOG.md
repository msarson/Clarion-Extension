...
- 🐛 **F12/hover on INCLUDE filenames resolves through the owning project's redirection first** (#328): in multi-project solutions where several projects redirect the same filename to different copies, F12 and filename hover walked projects in solution order and could open another project's copy. They now use the owning project's redirection first — the same #315 rule document links already follow — with the solution-order walk kept as fallback for ownerless files.
