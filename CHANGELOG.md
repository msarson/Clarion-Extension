...
- ✨ **Inlay hints**: inline **implicit-variable types** (`Counter#` → `: LONG`, `Pct$` → `: REAL`) and **parameter-name hints** at call sites (`Save(⟨pName:⟩ 'report', ⟨pFlags:⟩ 3)`) — for single-signature same-file `MAP` procedures and built-in functions (arity-disambiguated). Each type is independently toggleable via `clarion.inlayHints.parameterNames.enabled` / `clarion.inlayHints.implicitVariableTypes.enabled`, and all inlay hints respect VS Code's own `editor.inlayHints.enabled`. Enabled by upgrading the language client/server to LSP 8.x (which also unlocks other 3.17 features natively).
