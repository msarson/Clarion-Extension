...
- ✨ **Diagnostic: CLASS missing an implemented interface's method** (#165): a `CLASS,IMPLEMENTS(SomeInterface)` that omits one of the interface's declared methods now gets a Warning naming the missing method (previously the validator extracted the interface name and discarded it — a silent false-negative). v1 scope is conservative to avoid false positives: the INTERFACE must be declared in the same file, and classes with a base class are skipped (the method may be inherited). Cross-file interface enumeration and inherited-method resolution are follow-ups.
