...
- ✨ **Hover data taxonomy refactor — keywords / directives / built-ins are now separate files** (issue #77): `clarion-builtins.json` previously mixed actual Clarion built-in functions (ABS, CLIP, MESSAGE, OPEN, …) with control-flow keywords (IF, CASE, LOOP, RETURN, BREAK, CYCLE, …), program-structure keywords (PROCEDURE, FUNCTION, ROUTINE, MAP, MODULE, PROGRAM, CODE), OOP references (SELF, PARENT, NEW), operators (NOT, TIMES), and compiler directives (INCLUDE, COMPILE, OMIT, PRAGMA, EQUATE, MEMBER). 32 keyword entries moved to a new `clarion-keywords.json`; 6 directive entries moved into the existing `clarion-directives.json` (joining ITEMIZE / SECTION). New `KeywordService` mirrors `DirectiveService` (singleton, `isKeyword` / `getKeyword` / `getAllByPrefix`); the HoverRouter routes through `handleDirective` → `handleKeyword` → `handleBuiltin` so the same hover surface is preserved for every previously-recognised name. No user-visible change — hover still surfaces structured info (signature/syntax, description, category) for every migrated entry. The `MAP` overlap (it's both a structural keyword and a directive in some contexts) is owned by `clarion-keywords.json` because Clarion treats it as a structure declaration (`MyMap MAP ... END`); same for `MODULE`. Test invariant: a moved name is answered by exactly one of the three services. Pinned via 46 new unit tests in `KeywordService.test.ts`.
...
- ✨ **Labelled `LOOP` / `ACCEPT` + `BREAK <Label>` / `CYCLE <Label>` validation** (issue #65): the tokenizer + DocumentStructure now associate a leading column-0 OR indented label with its `LOOP` or `ACCEPT` structure (`Loop1 LOOP …`, `Outer ACCEPT …`), expose a new `resolveLoopLabel(name, fromLine)` API for innermost-match lookup (case-insensitive), and `ControlFlowDiagnostics` validates `BREAK` / `CYCLE` label arguments against in-scope labelled `LOOP` / `ACCEPT` structures. An unresolved label surfaces as a Warning: `Label 'X' does not refer to an enclosing LOOP or ACCEPT`. Unlabelled `BREAK` / `CYCLE` continue to follow the existing depth-counter rules. Existing single-line `LOOP / END` and unlabelled `ACCEPT / END` coverage is regression-pinned.

- ✨ **Diagnostic: VIEW `PROJECT(field)` validated against FROM file's RECORD** (Gap L follow-up): a new `view-project-unknown-field` Warning fires when a `VIEW(SomeFile)` structure projects a field name that doesn't exist on the FROM file's RECORD. Both bare (`Id`) and prefix-form (`Cus:Id`) names are matched case-insensitively. JOIN clause arguments are intentionally not validated — only `PROJECT(...)` is. v1 is single-document only: when the FROM file is declared in another file (the typical SV AppGen pattern) the validator skips silently, no false positives. Cross-file FROM resolution via `FileRelationshipGraph` is a follow-up. Built on the existing `ViewDescriptorParser` and `isFileRecord` parent-child marker; no DocumentStructure changes.

- ✨ **VIEW PROJECT/JOIN validator — cross-file FROM resolution + JOIN field validation** (task `d4fe847b`, follow-up to `7c2bd06`): two extensions over the v1 single-document validator. **(1) Cross-file FROM:** when `VIEW(SomeFile)` references a FILE not declared in the current document, the validator now walks the `INCLUDE`/`MEMBER` chain to locate it. Lookup loads target tokens via `TokenCache.getTokensByUri` first (covers open files with unsaved edits), then falls back to disk read + fresh tokenize. 1-hop fan-out — the included file's own INCLUDEs are walked one level deeper, which covers the common SV AppGen pattern (`MEMBER('parent.clw')` + `INCLUDE('files.inc')`) without unbounded graph traversal. **(2) JOIN field validation:** `JOIN(File, fieldRefs...)` clauses now validate every name token after the joined file against that file's RECORD fields, with the same cross-file resolution as FROM. `INNER JOIN` / `OUTER JOIN` are recognised via the bare `JOIN` keyword (the prefix sits earlier on the same line and is not part of the JOIN token itself). Both extensions inherit the v1 false-positive-trust contract: if the joined/FROM file can't be resolved (no INCLUDE chain reachable, file outside project, etc.) the validator skips silently rather than warning on a name it can't validate. New `JOIN field validation` and `Cross-file FROM resolution` test sub-suites pin the behaviour — 9 new sub-tests covering both extensions plus the silent-skip cases. The original v1 test suite (single-doc PROJECT validation) stays green unchanged.

- ✨ **Undeclared-variable diagnostic — dotted-access leading scope (`Obj.Field`)** (issue #62 v2, task `4a2ddc24` sub-feature 3, follow-up to `f161c40`): the diagnostic now flags the leading scope name in dotted-access expressions when that scope isn't declared. `BogusObj.Field = 1` warns on `BogusObj`; `LocalVar = BogusObj.Field.Deeper` warns on `BogusObj`; `IF BogusObj.Field > 0 THEN` warns on `BogusObj`. The diagnostic range covers ONLY the leading scope name, not the full `Obj.Field` token. Built-in scopes (`SELF.Method`, `PARENT.Init()`, etc.) are never flagged because the leading name is in `BUILT_IN_IDENTIFIERS`. The trailing member portion is intentionally NOT validated — member resolution lives elsewhere (DefinitionProvider, SymbolFinderService) and validating it here would require type-tracking. Forms that previously sat behind `containsSpecialChars` skip — prefixed (`Cus:Field`), indexed (`arr[i]`), field-equate (`?Ctrl`) — still skip even when followed by a dotted member: `Cus:Field.method`, `arr[1].member`, `?MyButton.Hide` are all unchanged from v1. Sub-feature 3 widens the safety net for tokenizer-split composites: the existing `isGluedNumberSuffix` source-text guard from sub-feature 1 is generalised to also detect Variable tokens whose preceding character is `.` (dotted-member continuation), preceding `:` (post-colon prefix-form), following `[` (indexed-access subject), or following `:` (pre-colon prefix-form). Two pre-existing tests (`Out-of-scope shapes > dotted member access LHS — skipped (has dot)` and `RHS expression validation > RHS dotted member — still skipped (sub-feature 3 will handle)`) updated to reflect the new behaviour: leading scope is now checked. With sub-feature 3, the v2 expansion of the validator (RHS / conditions / dotted) is complete; multi-line `|`-continuation and FOR-style `LOOP x = …` loop-variable handling remain deferred follow-ups.

- ✨ **Undeclared-variable diagnostic — IF / WHILE / UNTIL / CASE / OF / OROF / ELSIF condition expressions** (issue #62 v2, task `4a2ddc24` sub-feature 2, follow-up to `626480d`): the diagnostic now flags bare-identifier references inside condition expressions of conditional / loop / case structures. `IF Bogus > 0`, `LOOP WHILE Bogus`, `LOOP ... UNTIL Bogus`, `CASE Bogus`, `OF Bogus` / `OROF Bogus`, and `ELSIF Bogus` all warn when `Bogus` isn't declared in the current file. Filter contract is identical to the LHS / RHS checks: prefixed (`Cus:Field`), dotted (`obj.member`), indexed (`arr[i]`), and field-equate (`?Ctrl`) forms still skipped; built-ins (SELF/PARENT/RECORDS/…) still treated as declared; the same `declaredNames` set is consulted; the hex-suffix safety guard from sub-feature 1 still applies. The condition scan stops at the first `THEN` keyword on the line, so single-line `IF cond THEN stmt.` doesn't bleed into the trailing statement (trailing-statement coverage deferred). Multi-line `|`-continuation conditions are not yet handled. Detected the structural keywords by NAME (case-insensitive) rather than `TokenType`, because IF / CASE / LOOP tokenise as `TokenType.Structure` while ELSIF / OF / OROF / WHILE / UNTIL tokenise as `TokenType.Keyword` or `TokenType.ConditionalContinuation` — value-match is the simplest cross-type discriminator. Pre-existing IF-ELSE structure-validator test (`Should NOT flag IF-ELSE with single-line ELSE statements`) updated to filter out undeclared-variable diagnostics, preserving its narrow structure-validation intent.

- ✨ **Undeclared-variable diagnostic — RHS expressions on assignment lines** (issue #62 v2, task `4a2ddc24` sub-feature 1, follow-up to `a8a8e3e`): the diagnostic now flags bare-identifier references on the right-hand side of an assignment too — `MyVar = BogusName + 1` warns on both `MyVar` (LHS, v1) and `BogusName` (RHS, v2). Filter contract is identical to the LHS check: prefixed (`Cus:Field`), dotted (`obj.member`), indexed (`arr[i]`), and field-equate (`?Ctrl`) forms are still skipped (dotted-access narrowing is sub-feature 3); built-ins (SELF/PARENT/RECORDS/…) are still treated as declared; the same `declaredNames` set is consulted. Function calls (`SomeFunc(...)`) tokenise as `TokenType.Function` not Variable, so they're skipped automatically — call-site validation lives elsewhere. Multi-line `|`-continuation RHS expressions are not yet handled (single-line scan only); follow-up. **Hex / binary / octal numeric suffix safety:** the tokenizer doesn't recognise Clarion's `1000h` / `101b` / `17q` suffixes (it drops the leading number entirely and emits the suffix as a 1-char `TokenType.Variable` glued to the digits' position). The validator reads the source text immediately to the left of each candidate Variable and skips it when that character is a decimal digit — `pAdr = 1000h` no longer fires a false positive on `h`. Conditions (IF/WHILE/CASE) and dotted-access shapes are sub-features 2 and 3, shipping in adjacent commits.

- ✨ **Diagnostic: undeclared variable on LHS of assignment** (issue #62): raises a Warning on assignments like `Foo = 1` when `Foo` is not declared anywhere in the current file. **Enabled by default**; set `clarion.diagnostics.undeclaredVariables.enabled` to `false` (and reload the VS Code window) if false-positives surface in your codebase. v1 deliberately checks ONLY bare-identifier left-hand sides of assignments (including `+=`, `-=`, `*=`, `/=`); prefixed (`Cus:Field`), dotted (`obj.member`), indexed (`arr[i]`), and field-equate (`?Ctrl`) forms are intentionally not validated. Built-in Clarion identifiers (SELF, PARENT, RECORDS, ERRORCODE, …) are always treated as declared. Resolution is single-file only: cross-file globals declared in another MEMBER aren't resolved in v1 — disable the diagnostic if your codebase relies on cross-file globals until a future iteration adds reachability-aware lookup. Diagnostic code: `undeclared-variable`.

**Bug Fixes**

- 🐛 **MEMBER('parent.clw') file resolution in INCLUDE-statement commands routes through LSP redirection** (task `7cbf07f7`, file-finding audit follow-up B): the `addIncludeStatementToFile` MEMBER branch in `client/src/commands/IncludeStatementCommands.ts` previously did a sibling-only `path.join(currentDir, targetFile) + fs.existsSync` check. If the parent CLW lived in a RED-derived path (e.g. `[Common] *.clw = .\classes`) or in libsrc, the command threw "MEMBER file not found" even though the redirection parser would have resolved it. New `client/src/commands/memberResolution.ts` exports a vscode-free, dependency-injected `resolveMemberFile(target, currentFileFsPath, deps)` helper: tries LSP `clarion/findFile` first when the client is ready, falls back to the sibling probe otherwise (preserving single-file-without-loaded-solution mode that Mark relies on for ad-hoc CLW editing). LSP transport errors are swallowed so a flaky LSP doesn't block the sibling fallback. Helper lives in its own file so the client mocha runner — which excludes vscode-importing tests — can unit-test it. 5 regression tests in `IncludeStatementCommands.MemberResolution.test.ts` (Eve): LSP-routes-RED-paths bug pin, plus regression guards for LSP-not-ready, LSP-returns-null, both-miss-returns-null, and LSP-throws-swallowed-falls-back. Test interface assertions verify the LSP method name (`'clarion/findFile'`) and params shape (`{ filename }`) match the server handler at `server.ts:1359` literally. Helper test invocation: `npx mocha --ui tdd --timeout 30000 out/client/src/test/IncludeStatementCommands.MemberResolution.test.js`. Full suite 1380 passing.

- 🐛 **`SolutionManager.getEquatesTokens`: removed redundant libsrcPaths fallback** (task `9e580d19`, file-finding audit follow-up): the post-`findFile('equates.clw')` libsrc walk in `solutionManager.ts:364-370` re-probed the same `serverSettings.libsrcPaths` directories that `findFile`'s Tier 3 (b8b2d748) already covers. Removed the 7-line fallback and its now-unused `serverSettings` import. Updated the trailing log message from `'equates.clw not found via redirection or libsrcPaths'` to `'equates.clw not found via redirection'` — `via redirection` is accurate since libsrc resolution is now an internal concern of the redirection parser. No behavior change for any equates.clw resolution case (project-local .red, global .red + libsrc fallback, nothing-anywhere). 3 regression-guard tests in `RedirectionParser.EquatesResolution.test.ts` (Eve) prove all three resolution paths still work via the consolidated `findFile`. Full suite 1375 passing.

- 🐛 **Redirection parser: `findFile` honours active build configuration** (task `bd7e4a29`): `RedirectionFileParserServer.findFile` (sync) and `findFileAsync` (async) iterated `this.entries` with no `entry.section` filter, so `[Debug]`-only paths were searched under Release builds and vice versa. The companion `clarionProjectServer.getSearchPaths` (`:381-383`) and the file-mask resolver at `clarionProjectServer.ts:712` already filter with `entry.section === "Common" || entry.section === serverSettings.configuration` — `findFile` now mirrors that exact filter. Lookup-time (not parse-time) so a configuration switch on the same parser instance picks up the new active section without re-parsing — the alternative would be a surprising bug source where mid-session config changes still resolve against the parse-time configuration. The synthetic `*.* = ['.']` catch-all (post-01d635ef) is pushed with `section: "Common"` so it stays active in every configuration — no regression there. Free-form configuration string preserved — custom configs (e.g. `[Profile]`, `[Staging]`) work as soon as `serverSettings.configuration` carries the matching name. 14 regression tests in `RedirectionParser.BuildConfigFilter.test.ts` (Eve): Release/Debug skip-the-other-section bug pins (sync + async), `[Common]` and matching-section regression guards, configuration-switch-between-calls (sync + async — only passes under lookup-time filtering), plus hardcoding canaries with custom `[Profile]` configuration to prove the filter is dynamic and not enum-restricted to Debug/Release. Full suite 1372 passing.

- 🐛 **Redirection parser: multi-segment relative paths (`.\classes`, `.\SharedCode\equates`) resolve to project dir, not .red dir** (task `cfaa7584`): direct follow-up to `01d635ef`. The `!path.isAbsolute(dir)` "other relative" branch in `findFile` (sync) and `findFileAsync` (async) was intentionally left alone in 01d635ef per the minimal-fix scope — only bare `.`/`..` and the synthetic `*.*` catch-all were repointed at the project dir. Real-world .red entries like `*.clw = .\classes` and `*.inc = .\SharedCode\equates` fall into this "other relative" branch, and under a developer-modified global .red they were silently anchored on `<ClarionRoot>\bin\classes` instead of `<ProjDir>\classes`. Fix swaps `path.dirname(entry.redFile)` for `this.projectPath ?? path.dirname(entry.redFile)` in both branches — same shape as the 01d635ef `.`/`..` fix, now mechanically extended. The defensive `?? path.dirname(entry.redFile)` fallback preserves legacy behavior for any caller that invokes `findFile` without first running `parseRedFile` to set `projectPath`. Project-local .red is a no-op regression-wise (project-local .red sits in the project dir; the two anchors coincide). 6 regression tests in `RedirectionParser.OtherRelativePaths.test.ts` (Eve): single-segment `.\classes` sync + async, multi-segment `.\SharedCode\equates` sync + async, plus regression guards for project-local .red still resolving correctly. Full suite 1358 passing.

- 🐛 **Redirection parser: LIBSRC fallback layer wired into `findFile` / `findFileAsync`** (task `b8b2d748`): plain `findFile` consumers of `RedirectionFileParserServer` previously walked only the canonical Layer 1 (RED entries) and Layer 2 (project dir, via the `.` synthetic catch-all post-01d635ef); Layer 3 (`<libsrc>` paths from `ClarionProperties.xml`, populated into `serverSettings.libsrcPaths` by the client at `server.ts:1035`) was never consulted. The libsrc layer was hand-rolled at four call sites that needed it (`solutionManager.ts:364-372` for `equates.clw`, `StructureDeclarationIndexer.ts:279-280`, `ReferencesProvider.ts:356`, `RenameProvider.ts:222`), but anything calling plain `findFile` missed library files entirely. Fix adds a third tier in both `findFile` (sync) and `findFileAsync` (async): after the RED entries loop and the sourceFilePath sibling probe both miss, walk `serverSettings.libsrcPaths` in declared order, probing `path.join(libDir, filename)` and returning the first existing match. Layer ordering (RED + project beat libsrc) is preserved — the new tier runs only when both prior tiers miss. `checkedPaths` dedup applied across tiers so a libsrc dir that overlaps a RED entry is never probed twice. New `FilePathSource.LibSrc = "libsrc"` enum value distinguishes library-resolved hits from project / redirected ones for downstream consumers; existing source-field consumers are unaffected since the new value is additive. No-op when `libsrcPaths` is empty / unset (declared-default `[]`). Async tier walks paths sequentially rather than via `Promise.all` because declared-order priority matters and the fallback only runs after both higher tiers have already missed — the cost is bounded. 6 regression tests in `RedirectionParser.LibsrcFallback.test.ts` (Eve): sync + async libsrc fallback, sync + async priority guards (project wins over libsrc), multi-libsrc walk (lib1 empty → lib2 hits), and a "nothing anywhere → null" guard against over-eager probing. Full suite 1352 passing.

- 🐛 **Redirection parser: `.`/`..` and synthetic `*.*` catch-all resolve to project dir, not .red dir** (task `01d635ef`): `RedirectionFileParserServer.findFile` / `findFileAsync` previously resolved `.` and `..` entries against `path.dirname(entry.redFile)`, and `parseRedFileRecursive` / `parseRedFileRecursiveAsync` pushed a synthetic `*.* = [path.dirname(redFileToParse)]` catch-all at parse time. Both anchors are wrong per Clarion 11.1 docs (`redirection_file.htm`) and Mark's compiler trace — the project directory is the correct base. The bug was masked for project-local redirection files (project-local `.red` sits in the project dir, so the two coincide) but bit hard for the global-fallback path (`%ClarionRoot%\bin\Clarion110.red`) where lookups silently anchored on `…\bin\` instead of the user's project. Fix stores `projectPath` on the parser instance (set by `parseRedFile` / `parseRedFileAsync`) and uses it as the resolution anchor for `.`/`..` at lookup time; the synthetic catch-all entry now stores the literal `'.'` token rather than the .red dir, so it flows through the same lookup-time anchor. Entries themselves stay project-agnostic — the static `redFileCache` (keyed by .red path + mtime) remains safe to share across project parsers, since project context lives on the parser, not on cached entries. The "other relative" branch (`!path.isAbsolute(dir)` with `dir !== '.'/'..'`) is intentionally left anchored on the .red dir — not pinned by 01d635ef tests. 5 regression tests in `RedirectionParser.DotResolution.test.ts` (Eve): explicit `.` resolution sync + async, synthetic `*.*` catch-all sync + async, plus a fixture-control test asserting global-fallback selection. Full suite 1346 passing.

- 🐛 **Undeclared-variable diagnostic gate respects the constructor default for legacy clients** (issue #62 follow-up): the `clarion/updatePaths` notification handler used to unconditionally overwrite `serverSettings.undeclaredVariablesEnabled` based on the params field, which silently clobbered the now-`true` default with `false` whenever the client didn't include the field at all (`undefined === true → false`). Fixed: only an explicit boolean from the client wins; `undefined` preserves the constructor default. Added a searchable `[#62]` info-level breadcrumb at the validator entry so future "isn't firing?" reports can be diagnosed via the Clarion Language Server output channel (set log level to info).

- 🐛 **#62 stale-diagnostic observability hardening** (issue #62 investigation): the `[#62]` validator-entry breadcrumb in `UndeclaredVariableDiagnostics.ts` is promoted from `logger.info` to `logger.error` so it remains visible at the default release log level. Mark's repro of the time-dependent stale-diagnostic bug (declaration removed → diagnostic does not return until "later") needs a log capture across the broken→works transition; this change avoids requiring users to manually crank the Clarion log level to surface the trace. Pure observability change — no logic touched.

- 🐛 **#62 stale-diagnostic observability hardening — TokenCache trace lines** (issue #62 investigation, follow-up): the `[TC] Cache HIT` / `Cache MISS` / `Cache EMPTY` lines in `TokenCache.ts:getTokens` are promoted from `logger.info` to `logger.error` for the same reason as the `[#62]` breadcrumb above — they're load-bearing for diagnosing the time-dependent stale-diagnostic repro because they reveal whether the validator is reading fresh or cached tokens for each edit. The TokenCache module logger has `setLevel("error")` so info-level lines were previously filtered out even at the global default level. Pure observability change — TokenCache logic and incremental-tokenization path untouched.

- 🐛 **Undeclared-variable validator accepts `TokenType.Function` declarations** (issue #62, regression-prevention): the procedure-detection filter in `UndeclaredVariableDiagnostics.codeRanges` build loop used `t.type === TokenType.Procedure` only — same architectural class of latent bug that the unification commit addressed across ~30 sites in `ReturnValueDiagnostics`, `MapDeclarationDiagnostics`, and others. The validator was added after that sweep so it dodged the cleanup. Now uses `TokenHelper.isProcedureOrFunction(t)` for parity. In steady state `DocumentStructure.handleProcedureToken` force-normalizes `token.type` to `TokenType.Procedure` for both PROCEDURE and FUNCTION declarations, so this fix is purely defensive — any future code path that runs the validator on partially-processed tokens (or any future tokenizer change that preserves the `Function` type for declaration tokens) is protected. Not the cause of the time-dependent stale-diagnostic repro Mark is chasing — that investigation continues in a separate observability-hardening commit. No CHANGELOG-visible behaviour change in normal usage.

- 🐛 **#62 mode-C codeRanges=0 early-exit breadcrumb** (issue #62 investigation): the `validateUndeclaredVariables` early-exit at `if (codeRanges.length === 0) return diagnostics` previously returned silently — no breadcrumb, indistinguishable in the log from "the validator was never called for this URI." Mark's repro log captured for `SimpleNewSln.clw` (191/189 tokens) showed no `[#62] scanned ...` breadcrumb, but the file structure (all-in-one PROGRAM with inline procedures sitting AFTER the program's main CODE marker) is consistent with `DocumentStructure.process()` failing to populate `executionMarker` + `finishesAt` for those inline procedures, which would produce 0 codeRanges and trigger this exact silent path. New `[#62] early-exit: 0 code ranges in N tokens — uri=...` line at error level disambiguates the next repro capture: if it fires for the user's file, the bug is in `DocumentStructure.process()`'s handling of that procedure layout; if it doesn't, validateTextDocument is not being called for that URI at all. Pure observability — no logic change.

- 🐛 **#62 stale-diagnostic regression test pin — all-in-one PROGRAM shape with TokenCache divergence** (issue #62 investigation, RED test): full LSP log capture from Mark's repro (2026-05-04) decoded the actual bug: the validator IS being called for every edit on `SimpleNewSln.clw`, but at v14 it returns 0 diagnostics for content that should produce 1 (`MyVar` undeclared in `SomeProcedure` body). Fresh tokenization of the same final text correctly returns 1 — so the bug is **TokenCache's incremental tokenize producing stale state across small edits** specifically on the all-in-one PROGRAM shape (PROGRAM declarations + main CODE + multiple inline `PROCEDURE` definitions in the same file). New regression test in `DiagnosticProvider.test.ts` reproduces Mark's file shape exactly (51 lines, 4 inline procedures), simulates a char-by-char backspace cycle through `myvar long`, and asserts that cache result must match fresh-tokenize result at every intermediate version. Test goes **RED** as designed — pins the bug for routing rather than masking it. Reproduction shows cache=0 / fresh=1 at intermediate states `line="myv"` and `line="m"`. Hands the fix to Alice (TokenCache surface — `incrementalTokenize` for multi-procedure all-in-one PROGRAM files). Three additional fingerprint sub-tests (executionMarker, finishesAt, MyVar.line) ruled out hypothesis H2 (DocumentStructure boundary) — fresh tokenize of the same content produces correct executionMarker, finishesAt, and Variable.line for all 4 inline procedures, so the structure analyzer is fine in isolation; the divergence is in cache state.

- 🐛 **#62 stale-diagnostic root cause fixed — `DocumentStructure.process()` dispatch is now idempotent for PROCEDURE tokens** (issue #62, GREEN): turns out the divergence Charlie's RED test pinned wasn't a bug in `incrementalTokenize`'s merge logic — it was a bug in `DocumentStructure.process()` triggered BY incremental merging. `handleProcedureToken` mutates `token.type` from `Keyword` to `Procedure` on the first pass (so consumers can check `token.type === Procedure` without a subType lookup). `process()`'s top-level dispatch only enters its branch for `type === Keyword || ExecutionMarker`. After the first pass, PROCEDURE/FUNCTION tokens have `type === Procedure` — so when `incrementalTokenize` merges old (already-typed) tokens with newly-tokenized (Keyword-typed) tokens and runs `process()` over the merged array, only the freshly-retokenized PROCEDURE tokens hit `handleProcedureToken`. The kept old PROCEDURE tokens fall through every branch and are skipped entirely, which means `handleProcedureClosure` never fires when the iteration reaches a stale PROCEDURE token. The previous procedure stays on `procedureStack`, every subsequent CODE marker gets assigned to it, and `closeRemainingProcedures` extends its `finishesAt` to EOF. The diagnostic's `isInsideCode(line)` is still correct numerically (no false-true rejections of `MyVar`), but `declaredNames` ends up with the procedure's NAME-as-Label still in it from a stale execution-marker pass — so MYVAR gets matched against the wrong procedure's identity, the LHS-bare-identifier match silently passes, no diagnostic fires. Fix: include `TokenType.Procedure` in the dispatch gate alongside `Keyword`/`ExecutionMarker`. `handleProcedureToken` is now idempotent (subType/parent/label/finishesAt assignments all overwrite to the same value). 6 fingerprint sub-tests in the all-in-one PROGRAM shape suite go GREEN; 1276 tests pass total. Routine tokens are unaffected because `handleRoutineToken` does NOT mutate `token.type`. The latent double-process invariant warning at `TokenCache.ts:149-152` (task `1850456f`) is a separate hygiene issue (children-array `.push` in `processLabels` and `handleStructureToken` is still non-idempotent) — its visibility was masked by this dispatch bug.

- 🐛 **#62 diagnosis breadcrumbs demoted back to info** (issue #62 cleanup): now that the underlying bug is fixed in `aeb6cea` (DocumentStructure dispatch idempotency), the three diagnostic-time log promotions ship at info level — the original visibility — to avoid runtime noise on every validate/getTokens call. Affected lines: `[#62] scanned ... K diagnostics` (UndeclaredVariableDiagnostics.ts end-of-validator), `[#62] early-exit: 0 code ranges ...` (codeRanges=0 path), and `[TC] Cache HIT/MISS/EMPTY` (TokenCache.ts:getTokens). All three retain their text and call sites; only the level changes. Future incremental-tokenize or validator regressions can promote these temporarily during diagnosis without re-architecting the surface.

- 🐛 **`DocumentStructure.process()` children-array `.push` is now idempotent** (task `da2e9721`, hygiene follow-up to `aeb6cea`): the dispatch-gate fix in `aeb6cea` exposed a latent duplication path. `process()` legitimately runs more than once on the same token array — the tokenizer runs it once internally, and `TokenCache.incrementalTokenize` (plus `ScopeAnalyzer`, `MapProcedureResolver`, `AttributeDiagnostics`) constructs new `DocumentStructure` instances on those already-processed tokens and calls `process()` again. Pre-aeb6cea, the dispatch silently skipped already-typed PROCEDURE tokens, so the duplication was hidden for that subset; with the dispatch fix, every `parent.children.push(token)` site (~10) ran on each pass and grew children arrays unboundedly across edits. Doesn't affect the diagnostic (which iterates the flat token array), but consumers iterating `parent.children` (DocumentSymbolProvider, formatter, folding, hover, StructureDiagnostics, ClassDiagnostics, SymbolFinderService, SymbolHierarchyManager, DefinitionProvider) would have started producing duplicate symbols on edited files. Fix: introduced `private addChildOnce(parent, child)` that guards with `if (!parent.children.includes(child))` before pushing, and replaced the legacy `parent.children = parent.children || []; parent.children.push(token);` pattern at all 10 call sites (`processLabels`, `handleStructureToken` ×3, `processShorthandProcedures` ×2, `handleProcedureToken` ×2, `handleRoutineToken`, plus the column-0 Label fall-through in `process()`). New `Idempotency: process() can run more than once safely` suite under `DocumentStructure.ParentIndex.test.ts` goes RED on the unguarded pattern and GREEN with the helper — 4 sub-tests covering GROUP fields, MAP procedures, the Mark-style all-in-one PROGRAM shape, and three-pass duplication-doesn't-grow-unbounded. 1280 tests pass total. Memory-pinned debugging recipe + idempotency contract documented in `project_documentstructure_idempotency.md`.

- 🐛 **`MapDeclarationDiagnostics.validateMissingImplementations` resolves MODULE filenames before constructing cache URIs** (task `d2fadc09`): `Token.referencedFile` stores the unresolved bare filename from MAP MODULE declarations (`DocumentStructure.resolveFileReferences:1915` documents this contract). The MODULE branch of `validateMissingImplementations` was using it directly — `const clwPath = moduleToken.referencedFile!` — and then prefixing `file:///` for `TextDocument.create` / `TokenCache.getTokens`, which produced bare-filename URIs like `file:///OtherModule.clw` in the cache when the LSP server's CWD happened to be the project directory (where `fs.existsSync('OtherModule.clw')` succeeds). Those bare URIs duplicated the canonical `file:///c:/...` cache entry VS Code creates when the same file is opened, causing redundant tokenization on every validate pass and — under specific cache-state interleavings — diagnostic staleness when only one of the two cache entries refreshed. Fix mirrors the INCLUDE handler at lines 145-148: resolve the bare filename via `nodePath.join(currentClwDir, ...)` first (covers the same-directory MODULE case, which is the common shape), fall back to `resolveClwPath()` (redirection / project-dir / absolute / CWD), and skip the MODULE if neither finds it on disk. New regression test `MapDeclarationDiagnostics — MODULE filename URI resolution (d2fadc09)` writes a temp PROGRAM + MODULE pair, runs the validator, and asserts the resulting cache entry for the MODULE file is the full-path URI rather than the bare filename. `MapProcedureResolver` (lines 281, 319, 882) inspected and intentionally NOT bundled — it uses `Token.sourceFile` (already-resolved by INCLUDE expansion) and `resolvedPath` (locally resolved before URI construction), neither of which exhibits the bug. The orthogonal `f%3A` vs `f:` open→first-edit window dedup gap (related to `f347767`) is a separate task, not bundled here.

- 🐛 **Tokenizer recognises hex / octal / binary numeric literal suffixes** (task `7a98d63f`): the `Number` token pattern in `TokenPatterns.ts` previously matched only plain decimal literals (`/\b[0-9]+(\.[0-9]+)?\b/i`), which silently dropped the leading digits of any suffixed numeric literal. For `pAdr = 1000h` the `1000` Number token was never emitted; the tokenizer's character-class router fell through to the Variable pattern at the suffix character's source position and emitted a 1-char `h` `TokenType.Variable` — which the #62 v2 RHS expansion of `validateUndeclaredVariables` then flagged as a spurious undeclared-variable diagnostic. Alice mitigated downstream with a `isGluedNumberSuffix` source-text guard (commit `626480d`); this fix tackles the root cause in the tokenizer itself. New pattern: `/\b(?:[0-9][0-9A-Fa-f]*[hH]|[0-7]+[oO]|[01]+[bB]|[0-9]+(?:\.[0-9]+)?)\b/i`. The non-decimal forms all require a leading decimal digit per Clarion's lexer convention — `0FFh` is valid hex, `FFh` falls through to Variable (matching Clarion's actual lexer behaviour). Octal requires `[0-7]+[oO]` (`777o`, `12o`); binary requires `[01]+[bB]` (`1010b`). 12 regression tests in new `tokenizer-numeric-suffixes.test.ts`: 6 positive (hex `1000h` / `0FFh` / `0AFh`, octal `777o`, binary `1010b`, plain decimal `1.5` no-regression), 4 negative (bare `h` / `o` / `b` after whitespace stays Variable; `FFh` no-leading-digit stays Variable), 1 Alice-repro-shape (`pAdr = 1000h` produces no orphan suffix Variable). Alice's `isGluedNumberSuffix` is intentionally LEFT INTACT as defence-in-depth — its other four cases (dot/colon/index prefixes) remain load-bearing for unrelated tokenizer-split composite scenarios, and the digit-prev case is now redundant-but-harmless protecting against future tokenizer regressions on edge cases (string-literal-adjacent numeric, broken syntax). Suite 1341 passing.

- 🐛 **Canonical `file://` URI helper at every TokenCache-feeder construction site** (task `5b42b29b`): `d2fadc09` resolved bare filenames into full OS paths but the resulting URIs were still constructed by string-prefixing `'file:///' + path.replace(/\\/g, '/')` — on Windows that produces `file:///C:/...` (uppercase drive, unencoded colon), which doesn't match VS Code's canonical form `file:///c%3A/...`. Both URIs reference the same physical file, but `TokenCache` keyed them as separate entries — duplicating cache state during the open→first-edit window where the existing dedupe sweep at `server.ts:728-739` (commit `f347767`) hadn't yet had a chance to fire. New `server/src/utils/UriUtils.ts` exports `pathToCanonicalUri(absPath)` — a small, stateless helper that lowercases the drive letter and percent-encodes the colon for Windows paths, prepends `file:///` for POSIX. Idempotent on its own output. 7 unit tests in `UriUtils.test.ts` covering Windows uppercase/lowercase drive, forward-slash input, POSIX, bare filename, idempotency, and the `f:\Playground\SimpleNewSln\SimpleNewSln.clw` repro path. Sweep: 14 cache-feeder sites across 5 files (`CrossFileResolver` ×3, `MapDeclarationDiagnostics` ×4 + `clwFileUri` in code-action data, `IncludeVerifier` ×1, `ScopeAnalyzer` ×2, `MapProcedureResolver` ×4) replaced their `'file:///' + path.replace(...)` pattern with `pathToCanonicalUri(path)`. URIs returned in `Location.create()` for go-to-def navigation, hover content, and similar non-cache surfaces were intentionally left unchanged — Bob's surgical scope. The `f347767` post-edit dedupe sweep is retained as defence-in-depth: future contributors who add a new construction site without using the helper get caught by the sweep on the next edit, not the open. Augmented the `d2fadc09` regression test with a Windows-only encoding-shape assertion (`/^file:\/\/\/[a-z]%3A\//`); added a sibling test that asserts no two cache entries normalise to the same physical path after `validateMissingImplementations`. Suite 1298 passing.
...
- ⚡ **`TokenHelper.isProcedureOrFunction` — single source of truth across consumers**: replaces ~30 hand-rolled `t.type === TokenType.Procedure` filters across 16 consumer files (`DefinitionProvider`, `ReferencesProvider`, `HoverProvider`, `VariableHoverResolver`, `WordCompletionProvider`, `DocumentHighlightProvider`, `MemberLocatorService`, `MethodOverloadResolver`, `SymbolDefinitionResolver`, `ClarionFormatter`, `ReturnValueDiagnostics`, `MapDeclarationDiagnostics`, `MapDeclarationCodeActionProvider`, `MapModuleCodeActionProvider`, `ProcedureUtils`, `SymbolFinderService`) with a single `TokenHelper.isProcedureOrFunction(token)` predicate that accepts both `TokenType.Procedure` and `TokenType.Function`. Modern Clarion treats `PROCEDURE` and `FUNCTION` as semantically identical (both can return values); the type-token split is a tokenizer artifact. Eliminates a class of latent bugs where any FUNCTION-typed declaration was silently filtered out — "Find All References", "Go to Definition", hover, completions, return-value diagnostics, MAP-mismatch diagnostics, and method overload resolution now all see FUNCTION declarations alongside PROCEDURE ones. Tokenizer assignment paths and hover-wording branches that intentionally distinguish the two are left alone.

- ⚡ **Quieter language-server logs**: noisy `logger.warn` performance traces in `StructureDeclarationIndexer` (3 sites) and `logger.error` info traces in `FileRelationshipGraph` (2 sites) and `IncludeVerifier` (16 sites) demoted to `logger.debug`. Catch-block error sites are unchanged — real failures still surface at error level. Reduces signal-to-noise in the LSP output channel during normal solution loads.
...
- 🐛 **Rename / Definition / Hover miss FUNCTION-typed procedure declarations** (hotfix `4d1435b1`): `Foo FUNCTION(LONG),REAL` declarations were silently invisible to F2 / Go to Definition / Hover when invoked from a call site. Root cause: `SymbolFinderService.findProcedureDeclaration` filtered tokens by `t.type === TokenType.Procedure` only, rejecting the tokenizer's separate `TokenType.Function` typing. Modern Clarion's PROCEDURE and FUNCTION are semantically identical; the gate now accepts both. Same gate applied to call-site identification on PROCEDURE/FUNCTION lines.

- 🐛 **`prepareRename` rejects cross-file procedure call sites** (hotfix follow-up): F2 on a procedure call whose declaration lived in another file (e.g. caller in `caller.clw`, declaration in `proc.clw` reached via `MAP/MODULE`) failed with `Cannot rename '<name>': symbol not found or not renameable`, because `prepareRename` validated via the per-file `SymbolFinderService` while the actual rename in `provideRename` was solution-aware via `ReferencesProvider`. `prepareRename` now falls back to `ReferencesProvider.provideReferences` when the per-file resolver returns no symbol — any non-empty result confirms the rename will succeed and the validation passes.

- 🐛 **Bare `WINDOW` / `APPLICATION` / `MENU` / `TOOLBAR` keywords tokenize incorrectly** (`BareWin WINDOW` with no parens, no attributes — and the equivalent for the other three): each keyword's regex required a `(` or `,` lookahead. The bare form failed the lookahead — for `WINDOW` the tokenizer's no-match advance skipped the leading `W` and the residual `INDOW` matched as a Variable; for `APPLICATION`/`MENU`/`TOOLBAR` (which are not in the Variable regex's exclusion list) the keyword itself tokenized as a plain Variable. Either way the structure was lost. The lookahead now also accepts whitespace-followed-by-end-of-line and `!` (comment), so the bare forms tokenize correctly while declarations with attributes (`WINDOW('Title'),AT(…)`) remain unchanged.

- 🐛 **Report-band keywords `DETAIL` / `HEADER` / `FOOTER` / `FORM` not classified as controls inside REPORT bodies** (Gap J follow-up): `clarion-controls.json` lists these four as `reportControls` and `ControlService.isControl` accepts them, but the tokenizer's `isDeclarationStructure` early-exit gate didn't list them, so the `STRUCTURE_PATTERNS` loop never ran on those keywords. They tokenized as something other than `Structure`, and `getControlContextAt` therefore couldn't surface them as controls — breaking attribute-completion and diagnostic context detection inside REPORT bodies. Added DETAIL/HEADER/FOOTER/FORM to the declaration-structure whitelist, mirroring the ITEMIZE alignment shipped with Gap B.
