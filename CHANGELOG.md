...
- **Dot completion on a FILE label offers its fields, and structure completion lists bare field names.** `Orders.` lists the same fields, keys and record group as `ORD:`, letters typed after the dot narrow the list, and on both paths each item is labelled by its field name with the qualified name (`ORD:CusID`) alongside. Previously only GROUP, QUEUE and RECORD labels answered a dot. [#505](https://github.com/msarson/Clarion-Extension/issues/505), [#507](https://github.com/msarson/Clarion-Extension/issues/507)
