...
- **Experimental: a report of the file references that do not resolve.** **Clarion: Unresolved File References (experimental)**, or a click on the Graph row in the Clarion Tools pane, lists every INCLUDE, MEMBER and MODULE target the file graph could not find, with the file and line, plus project sources that are missing. References inside OMIT or COMPILE blocks and MODULE names with no extension are listed separately. [#687](https://github.com/msarson/Clarion-Extension/issues/687)
