...
- ✨ **Refactor: Introduce EQUATE** (#281): with the cursor on (or a selection of) a numeric or string literal, a **Ctrl+.** refactor **"Introduce EQUATE"** extracts the magic literal to a named `EQUATE`. Mirroring Surround With, it then asks — as a second step — **which data section** to declare it in (a quick pick: the routine's `DATA` when the cursor is in a routine that has one, the enclosing procedure's local data, and the file-level section — **module** data in a `MEMBER`, **global** data in a `PROGRAM`), then prompts for the name. The `EQUATE` is inserted in the chosen scope (label at column 0) and the literal is replaced with the name. Insertion points come from `DocumentStructure`, so scope detection is structural rather than textual. Pictures (`@…`) and replace-all-in-scope are follow-ups.
