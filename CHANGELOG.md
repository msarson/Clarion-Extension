...
- **Nine documented built-in functions were added to the built-in catalog** — `COMPRESS`, `DEBUGHOOK`, `DECOMPRESS`, `HTTPWEBREQUEST`, `HTTPWEBREQUESTtoFile`, `ImageRotateFlip`, `ImageToPng`, `PRINTERDIALOGA` and `SETLAYOUT` — so they are recognised by completion, hover and diagnostics rather than treated as unknown. [#519](https://github.com/msarson/Clarion-Extension/issues/519)
