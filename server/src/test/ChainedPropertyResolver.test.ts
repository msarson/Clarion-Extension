...
suite('extractClassName (moved from ClassMemberResolver, #625)', () => {

    test('strips & reference prefix', () => {
        assert.strictEqual(extractClassName('&SortOrder'), 'SortOrder');
    });

    test('strips & and trailing attributes', () => {
        assert.strictEqual(extractClassName('&SortOrder,PROTECTED'), 'SortOrder');
    });

    test('strips attributes without &', () => {
        assert.strictEqual(extractClassName('SortOrder,DIM(10)'), 'SortOrder');
    });

    test('strips DIM() without comma', () => {
        assert.strictEqual(extractClassName('MyQueue(BaseQueue)'), 'MyQueue');
    });

    test('returns null for PROCEDURE', () => {
        assert.strictEqual(extractClassName('PROCEDURE'), null);
    });

    test('returns null for PROCEDURE with params', () => {
        assert.strictEqual(extractClassName('PROCEDURE(STRING,LONG)'), null);
    });

    test('returns null for STRING primitive', () => {
        assert.strictEqual(extractClassName('STRING(20)'), null);
    });

    test('returns null for LONG', () => {
        assert.strictEqual(extractClassName('LONG'), null);
    });

    test('returns null for SHORT', () => {
        assert.strictEqual(extractClassName('SHORT'), null);
    });

    test('returns null for BYTE', () => {
        assert.strictEqual(extractClassName('BYTE'), null);
    });

    test('returns null for QUEUE keyword alone (not a named queue type)', () => {
        assert.strictEqual(extractClassName('QUEUE'), null);
    });

    test('returns null for GROUP keyword alone', () => {
        assert.strictEqual(extractClassName('GROUP'), null);
    });

    test('returns null for empty string', () => {
        assert.strictEqual(extractClassName(''), null);
    });
    ...
    test('returns null for Unknown', () => {
        ...
        assert.notStrictEqual(extractClassName('Unknown'), null);
    });

    test('preserves case of class name', () => {
        assert.strictEqual(extractClassName('&BufferedPairsClass,PRIVATE'), 'BufferedPairsClass');
    });
    ...
    test('LIKE(TypeName) — returns the referenced type name', () => {
        assert.strictEqual(extractClassName('LIKE(WindowPositionGroup)'), 'WindowPositionGroup');
    });

    test('LIKE(TypeName) with trailing attributes', () => {
        assert.strictEqual(extractClassName('LIKE(WindowPositionGroup),PRIVATE'), 'WindowPositionGroup');
    });

    test('LIKE(PREFIX:TypeName) — supports colon-qualified names', () => {
        assert.strictEqual(extractClassName('LIKE(PYA:RECORD)'), 'PYA:RECORD');
    });

    test('LIKE alone (no parens) — returns null', () => {
        assert.strictEqual(extractClassName('LIKE'), null);
    });

    test('GROUP(TypeName) — returns the referenced type name', () => {
        assert.strictEqual(extractClassName('GROUP(ConnectionSettingsType)'), 'ConnectionSettingsType');
    });
    ...
    test('GROUP(TypeName) with trailing " END" (verbatim scanned declaration text) — real repro shape', () => {
        ...
        assert.strictEqual(extractClassName('GROUP(ConnectionSettingsType) END'), 'ConnectionSettingsType');
    });

    test('QUEUE(TypeName) — returns the referenced type name', () => {
        assert.strictEqual(extractClassName('QUEUE(BaseQueueType)'), 'BaseQueueType');
    });

    test('RECORD(TypeName) — returns the referenced type name', () => {
        assert.strictEqual(extractClassName('RECORD(SomeRecordType)'), 'SomeRecordType');
    });

    test('GROUP(TypeName) with trailing attributes after the paren', () => {
        assert.strictEqual(extractClassName('GROUP(FooType),DIM(2)'), 'FooType');
    });
