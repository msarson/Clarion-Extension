...
suite('ReferenceCountIndex #315 — qualifier-scoped counts + FAR pruning', () => {
    ...
    test('verifyFilesFresh: batched async pass re-scans regenerated files and bounds later stats', async () => {
        const regen2 = writeFixture('regen2.clw', '  CODE\n  Nothing2()\n');
        const idx = ReferenceCountIndex.getInstance();
        await idx.buildInBackground([...files, regen2]);
        assert.strictEqual(idx.mayContain(regen2, 'ThisGPF'), false);

        fs.writeFileSync(regen2, '  CODE\n  ThisGPF.Initialize()\n');
        const bumped = new Date(Date.now() + 5_000);
        fs.utimesSync(regen2, bumped, bumped);

        await idx.verifyFilesFresh([...files, regen2]);
        assert.strictEqual(idx.mayContain(regen2, 'ThisGPF'), true,
            'regenerated content picked up by the batched pass');
        assert.strictEqual(idx.getCount('ThisGPF.Initialize'), 6, 'counts re-scanned from fresh content');
    });
...
suite('FileRelationshipGraph #315 — MEMBER behind a template banner still edges', () => {
    ...
    setup(() => {
        ...
        fs.writeFileSync(path.join(dir, 'memb1.clw'), [
            '!ABCIncludeFile',
    ...
    test('cold scan records the MEMBER edge when a banner precedes it', async () => {
        const { FileRelationshipGraph } = require('../FileRelationshipGraph') as typeof import('../FileRelationshipGraph');
        ...
        assert.ok((graph.lastBuildStats?.memberEdges ?? 0) >= 1, 'build stats count the member edge');
    });

    test('a file whose first statement is not MEMBER gets no member edge', async () => {
