...
(async () => {
    ...
    for (const p of positions) {
        ...
        out.push({ key: `${p.file}:${p.line}:${p.character}`, word: p.word, slice: p.slice, hover: text(hover), f12: locs(def) });
    }
    ...
    if (arg('against')) {
        ...
        let hoverMoved = 0, f12Moved = 0;
        for (const r of out) {
            const b = prev.get(r.key);
            if (!b) continue;
            const h = b.hover !== r.hover, f = b.f12 !== r.f12;
            if (h) hoverMoved++;
            if (f) f12Moved++;
            if (h || f) {
                console.log(`\n== ${r.slice} ${r.word}  ${r.key.split('/').pop()}${f ? `   F12 ${b.f12} -> ${r.f12}` : ''}`);
                if (h) console.log(`  before: ${b.hover.replace(/\n+/g, ' | ').slice(0, 260)}\n  after:  ${r.hover.replace(/\n+/g, ' | ').slice(0, 260)}`);
            }
        }
        console.log(`\nhover cards changed: ${hoverMoved}   F12 targets changed: ${f12Moved}   (of ${out.length})`);
    }
    await session.close();
